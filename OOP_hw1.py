    ...
    def rate_lecturer(self, lecturer, course, grade):
        ...
        for key in lecturer.grades.keys():
            for grad in list(lecturer.grades[key]):
                sum_lecturer = sum_lecturer + grad
                len_lecturer += 1
        lecturer.average_rating = round(sum_lecturer / len_lecturer, 2)
    ...
    def rate_hw(self, student, course, grade):
        ...
        for key in student.grades.keys():
            for grad in list(student.grades[key]):
                sum_hw = sum_hw + grad
                len_hw += 1
        student.average_rating = round(sum_hw / len_hw, 2)
...
first_student = Student("Исаак", "Ньютон", "м")
first_student.finished_courses += ["Physics"]
first_student.courses_in_progress += ["GIT"]
first_student.courses_in_progress += ["Python"]

second_student = Student("Никола", "Тесла", "м")
second_student.courses_in_progress += ["Python"]
second_student.finished_courses += ["Electrician"]

first_lecturer = Lecturer("Галилей", "Галилео")
first_lecturer.courses_attached += ["Python"]
first_lecturer.courses_attached += ["GIT"]

second_lecturer = Lecturer("Томас", "Эдисон")
second_lecturer.courses_attached += ["GIT"]

first_reviewer = Reviewer("Аристотель", "Древнегреческий")
first_reviewer.courses_attached += ["Python"]

second_reviewer = Reviewer("Александр", "Лодыгин")
